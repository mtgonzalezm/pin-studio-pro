...
export default function PinStudio() {
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      hotspots.forEach((h) => {
        ...
        ctx.strokeStyle = selectedHotspot?.id === h.id ? '#00BDAA' : 'rgba(242, 96, 118, 0.3)';
  ...
  if (!image) {
    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #00BDAA 0%, #A1DD70 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
        <div style={{ background: 'white', borderRadius: '30px', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)', padding: '48px', maxWidth: '500px', width: '100%', textAlign: 'center' }}>
          ...
          <button onClick={() => fileInputRef.current?.click()} style={{ background: 'linear-gradient(to right, #00BDAA, #A1DD70)', color: 'white', fontWeight: 700, padding: '16px', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', width: '100%', border: 'none', cursor: 'pointer', fontSize: '16px', marginBottom: '16px' }}>
        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
          ...
          {!studentMode && (
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              <button onClick={() => setIsTransparentMode(!isTransparentMode)} style={{ background: isTransparentMode ? '#00BDAA' : '#e5e7eb', color: isTransparentMode ? 'white' : '#1f2937', padding: '8px 16px', borderRadius: '6px', fontWeight: 600, border: 'none', cursor: 'pointer' }}>
                👻 {isTransparentMode ? 'Zona Transparente (activa)' : 'Zona Transparente'}
              </button>

              <div style={{ display: 'flex', gap: '8px', marginLeft: 'auto' }}>
                <button onClick={() => setDrawMode(drawMode === 'rect' ? null : 'rect')} style={{ padding: '8px 16px', borderRadius: '6px', fontWeight: 600, border: '2px solid', borderColor: drawMode === 'rect' ? '#00BDAA' : '#d1d5db', background: drawMode === 'rect' ? '#00BDAA' : 'white', color: drawMode === 'rect' ? 'white' : '#1f2937', cursor: 'pointer' }}>
                  ▭ Rectángulo
                </button>
                <button onClick={() => setDrawMode(drawMode === 'circle' ? null : 'circle')} style={{ padding: '8px 16px', borderRadius: '6px', fontWeight: 600, border: '2px solid', borderColor: drawMode === 'circle' ? '#00BDAA' : '#d1d5db', background: drawMode === 'circle' ? '#00BDAA' : 'white', color: drawMode === 'circle' ? 'white' : '#1f2937', cursor: 'pointer' }}>
                  ◯ Círculo
                </button>
              </div>
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            <div style={{ background: 'white', borderRadius: '12px', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)', padding: '24px' }}>
              ...
              <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
                {hotspots.map((h) => (
                  <div key={h.id} onClick={() => setSelectedHotspot(h)} style={{ padding: '12px', borderRadius: '8px', cursor: 'pointer', marginBottom: '8px', background: selectedHotspot?.id === h.id ? '#fef3c7' : '#f9fafb', border: '2px solid', borderColor: selectedHotspot?.id === h.id ? '#00BDAA' : '#e5e7eb' }}>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              <button onClick={() => setShowPreview(true)} style={{ flex: 1, background: '#8559A5', color: '#1f2937', padding: '10px', borderRadius: '6px', fontWeight: 600, border: 'none', cursor: 'pointer' }}>
                👁️ Vista previa
              </button>
              <button onClick={exportJSON} style={{ flex: 1, background: '#00BDAA', color: 'white', padding: '10px', borderRadius: '6px', fontWeight: 600, border: 'none', cursor: 'pointer' }}>
                💾 JSON
              </button>
              <button onClick={exportHTML} style={{ flex: 1, background: '#00BDAA', color: 'white', padding: '10px', borderRadius: '6px', fontWeight: 600, border: 'none', cursor: 'pointer' }}>
                🌐 HTML
              </button>
            </div>
          </div>
